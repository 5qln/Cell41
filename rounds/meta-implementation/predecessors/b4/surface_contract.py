#!/usr/bin/env python3
"""surface_contract — B4's contract seam: the attested rounds read by
path, sha-pinned, and the unattended run's surface declared against them.

The attested sides meet at one contract: P4a's ``surface.py`` declares the
§3.6 shape, P4b's grammar renders the desk bundles against it, B2's driver
owns the prompt→fence→read→propose mechanics, B3's descent owns the
addressing convention, and B0's ledger owns the record chain.  This module
does not re-declare, re-invent or fork any of them — it reads each by file
path, pins the exact bytes it imported (sha256), and then declares the
RUN surface (the record conventions, the trail contract, the dependency
audit, the guard policy, the cost contract, and the folded desk
function-specs) against that contract, in one place, versioned.
...
decoding operations quoted byte-faithful, sourced from
``fixtures/desk.py`` (pinned, stdlib-only) so the fixture desk subprocess
can import the same bytes without pulling this module's heavy loaders.
"""
...
# identical — the sha pins below are the contract either way).
# ---------------------------------------------------------------------------
...
PINNED_FILES = (
    ...
    },
    {
        ...
        "role": "B2 the herdr socket adapter (the attested prompt→fence→read)",
    },
    {
        "path": os.path.join(_B2_DIR, "lens.py"),
        "sha256": "ad46b895dc3ceb68379467d8c9b642affcfc1b214633a1de9f89d39240fd269a",
        "role": "B2 the Pi lens (never invoked — the fixture stand-in lens replaces it, H-B4-1)",
    },
    {
    ...
    },
    {
        ...
        "role": "P4b desk grammar (COURSE, DESK_GATES, EQUATION_FORMS, seat_address, render_bundle)",
    },
    {
    ...
    # This round's fixture desk — the folded item's byte-faithful carrier.
    {
        "path": os.path.join(_HERE, "fixtures", "desk.py"),
        "sha256": "0d8c47fd90a69a47107b78c143f4a59c33d4468ee5ce18f52446f1119303a06d",
        "role": "B4 fixture desk — the codex §2 desk function-specs, byte-faithful (the folded item)",
    },
)
...
# -- the fixture desk's folded specs (stdlib-only; pinned above) -----------
_desk = _load_pinned(_pin_for(os.path.join(_HERE, "fixtures", "desk.py")),
                     "desk_specs", path_entries=(os.path.join(_HERE, "fixtures"),))
...
# ---------------------------------------------------------------------------
# The run's declared surface — the guard policy, the record conventions, the
# trail contract, the audit, the budget.  Every field carries its citation;
# the declaration is data, never logic (B3's DESCENT_SURFACE precedent).
# ---------------------------------------------------------------------------
...
RUN_SURFACE = {
    ...
    "round": "R05-B4",
    "what": (
        "the unattended run: holds accumulate instead of stopping the run; "
        "TENTATIVE seeding of the next S; restart re-arm from the ledger "
        "alone; budget hold (a spend ceiling surfaces as a held gate, "
        "never a silent kill); a readable trail while it runs (PRD §B4, "
        "C1–C7)"),
    "cells": {
    ...
    "records": {
        ...
        "hold": ("the gate that failed to lock: held-pending, "
                 ...
                 "kinds: outage (adapter error) · blocked (the desk "
                 "announced no surface) · guard-fail (a DESK-FIDELITY "
                 "FAIL) · budget-ceiling (the run-ending stop).  A hold "
                 "is never auto-resolved: no code path writes attested "
                 "or attestation_ref on any machine record (T-O5-02)"),
        "payload_ref": {
    ...
    "budget": {
        "rule": ("model spend is accounted BEFORE each turn: the charge "
                 ...
                 "(C4, PRD §10.3)"),
        "charges": ("cost.COST_MODEL['charges'] — declared data, "
                    "conservative (each charge ≥ the measured turn cost; "
                    "the live per-Pi measurement awaits a constituted "
                    "desk, H-B4-2)"),
    },
    "trail": {
    ...
    "folded_item": {
        "rule": ("the five desk function-specs are the codex §2 decoding "
                 ...
                 "sourced from fixtures/desk.py); no new decoding "
