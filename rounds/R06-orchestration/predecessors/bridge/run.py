...
extended by the bridge: the live desk mode, C1/C2, and the runtime
config-read, C3/C4).
...
writes the observability trail (the pinned B4 trail.py): append-only,
hash-chained, replayable, readable mid-run, decoding-not-transcript
(C7).

The bridge's two extensions, in one sentence each:

  * LIVE desk mode (C1/C2): ``cost.DeskAdapter``'s third mode joins the
    turn to the live herdr socket — ``open_turn`` returns a TurnContext
    on the resolved live socket (HERDR_SOCKET_PATH, else
    ~/.config/herdr/herdr.sock) with process None, and the SAME
    Instrument path B4 built (Instrument(socket_path=context.socket_path)
    → prompt_desk) now speaks the REAL herdr dialect through the
    attested B2 adapter: label-resolve every turn, agent.prompt to the
    resolved pane, fenced read to ⟦END …⟧.  No fixture desk_server.py is
    ever spawned in this mode; an unreachable socket holds as outage, a
    no-agent desk (agent_not_found) holds as blocked — never a fixture
    stand-in, never a guessed answer, never clean (C2, lens 6).  The
    centre guard (S / podium) refuses before any byte (the imported
    assert_not_centre, K4).

  * runtime config-read (C3/C4): the prompt and budget paths read
    THROUGH softconfig — each desk's codex §2 emphasis, voice and model
    and the declared charges + default mode come from a soft-layer
    config file at runtime; the hard-coded DESK_FUNCTION_SPECS /
    COST_MODEL literals are the DECLARED DEFAULTS (carried in
    softconfig.SOFT_DEFAULTS and cost.COST_MODEL — data, never this
    module's control flow).  Absent soft config → the declared defaults
    (B4's exact bytes/values: the fixture run is unchanged, C6).
    Malformed or partial soft config → the run refuses to boot with the
    reason — INCONCLUSIVE, never a silently substituted value.
...
the seating convention); B4's trail and fixture desk (the pinned
trail.py and the byte-faithful folded item).  The herdr socket dialect,
the D.12 checks, the desk grammar and the descent are never
re-implemented here.
...
root, no leaf).  Deterministic and stdlib-only: the live socket is the
only external I/O, and it is the attested instrument's (K1).
"""
...
import softconfig  # noqa: E402  (the runtime config-read — loaded and
                                 #  sha-pinned by surface_contract)
import trail  # noqa: E402  (the pinned B4 trail — loaded by
                            #  surface_contract)
...
class BootError(Exception):
    ...
    the chain is broken, the spec is not lawful, or the soft config is
    INCONCLUSIVE — nothing ran, never a substituted value."""
...
class Conductor(Driver):
    ...
    the audit, the trail — plus the bridge's live desk mode (C1/C2) and
    the runtime config-read (C3/C4).
    ...
    the same bytes.  The prompt and budget paths read through
    ``self.soft`` (the softconfig view, loaded from disk — the spec's
    "soft_config" path, the SOFT_CONFIG_PATH env var, or the declared
    default location), so a fresh process rebuilds the live mode AND the
    config-read from disk alone (C7).
    """
    ...
    def __init__(self, ledger_path, trail_path, spec, socket_dir=None,
        ...
        # the soft layer, read at runtime (C3/C4): the spec's
        # "soft_config" path (caller-supplied data), else the
        # SOFT_CONFIG_PATH env var, else the declared default location.
        # Absent file -> the declared defaults (B4's exact bytes).
        # Empty / malformed / partial -> INCONCLUSIVE with the reason —
        # the run refuses to boot, never a silently substituted value.
        soft_path = spec.get("soft_config")
        if soft_path is not None and not isinstance(soft_path, str):
            raise BootError("the spec's soft_config must be a path or null")
        self.soft = softconfig.load_soft_config(soft_path)
        if self.soft.get("status") == "inconclusive":
            raise BootError(
                "the soft config %r is INCONCLUSIVE — never a silently "
                "substituted value: %s"
                % (self.soft.get("path"), self.soft.get("reason")))
        # the mode default is DECLARED DATA — resolved through the soft
        # layer (softconfig's declared default is cost.COST_MODEL's
        # table entry, byte-exact — H-B4-2 carried; the spec and the
        # soft config may each override, never a mode literal here)
        ...
        self.mode = mode if mode is not None else (
            ...
            else softconfig.default_mode(self.soft))
    ...
    def _source_ref_for_seed(cls, spec, authority, cell, cycle, plant,
        ...
        from the spec's template — the same bytes the fixture desk
        answered).

        In LIVE mode the V answer's bytes exist nowhere on disk (D12:
        references, never content) — so the seed's source reference is
        the previous cycle's V turn record's own payload_ref
        (fenced:sha256:<digest>), re-derived from the ledger alone,
        never regenerated, never guessed (C7)."""
        if mode == "live":
            prev_v = cls._turn_record(authority, seats["V"], "b",
                                      "cycle:%d" % (cycle - 1),
                                      block_version)
            if prev_v is None:
                raise BootError("the V answer of cycle %d is not on the "
                                "ledger — the live seed has no reference "
                                "to carry" % (cycle - 1,))
            return prev_v["payload_ref"]
        if cycle == 0:
    ...
    def _regenerate_answer(cls, spec, cell, cycle, desk):
        ...
        server answered).  The pinned B4 fixture desk composes it
        (imported via surface_contract — never re-authored)."""
        ...
        return compose_answer(template, cell, cycle, desk, marker)
            ...
            for cell in self.cells:
                ...
                for desk in DESKS_WALKED:
                    ...
                    if not father_ok:
                        suspended = True
                        break
                    # the budget path reads through the soft layer (C3):
                    # the spend from the completed records with the soft
                    # charges, and the due charge from softconfig —
                    # never a charge literal in this module's flow
                    spend = cost.spend_from_records(
                        records, self.mode, charge_for=self._soft_charge)
                    charge = softconfig.budget_of(self.soft, self.mode,
                                                  desk)
    ...
    #    the soft fields read through softconfig — never a spec literal
    #    in this module's control flow, C3) --------------------------------

    def _soft_charge(self, mode, desk):
        """The budget path's charge resolver — the soft layer's value
        for (mode, desk), never a literal here (C3)."""
        return softconfig.budget_of(self.soft, mode, desk)

    def _prompt_text(self, cell, cycle, desk, question_ref, key):
        emphasis = softconfig.desk_emphasis(self.soft, desk)
        voice = softconfig.desk_voice(self.soft, desk)
        model = softconfig.desk_model(self.soft, desk)
        lines = ["⟦TURN cell=%s cycle=%d desk=%s⟧" % (cell, cycle, desk),
                 "⟦DESK FUNCTION-SPEC — codex §2, attention mode⟧",
                 FOUNDING_SENTENCE,
                 "ATTENTION MODE — %s" % voice]
        lines.extend(emphasis)
        # the model line renders in live mode, and whenever a soft layer
        # is present — so the model's runtime read is observable; absent
        # soft config + fixture mode the prompt stays B4's exact bytes
        # (C4/C6: the model line never existed in B4's prompt).
        if self.mode == "live" or self.soft.get("status") == "ok":
    ...
    def _do_turn(self, action):
        ...
        try:
            ...
            # C2: the live desk resolved by label to a pane with NO
            # agent — a blocked hold, never a fixture stand-in, never a
            # guessed answer, never clean.
            self._desks_adapter().close_turn(context)
            hold_action = self._hold_action(
                cell, cycle, desk, addr, gate, self._hold_key(addr, gate),
                "blocked", "agent_not_found", None)
            return self._record_hold(hold_action)
        except HerdrError as exc:
            # an unreachable live socket lands here too (C2): an outage
            # hold, never a fixture stand-in.
            self._desks_adapter().close_turn(context)
            hold_action = self._hold_action(
                cell, cycle, desk, addr, gate, self._hold_key(addr, gate),
    ...
    def _rebuild_seed_line(self, record):
        ...
        source_ref = self._source_ref_for_seed(
            ...
            mode=self.mode, block_version=self.block_version)
    ...
    def _rebuild_hold_line(self, record):
        ...
        if (hold_kind == "guard-fail" and detail != "surface-malformed"
                and self.mode != "live"):
            # the live answer's bytes are never retained — the guard
            # report cannot be re-evaluated from disk; the rebuilt line
            # carries the hold honestly, never a guessed report
    ...
    def _rebuild_turn_line(self, record):
        if self.mode == "live":
            return self._rebuild_live_turn_line(record)
        authority = self._authority()
        ...
        charge = softconfig.budget_of(self.soft, self.mode, desk)
    ...
    def _rebuild_live_turn_line(self, record):
        """The kill -9 repair in LIVE mode: the live answer's bytes were
        never retained (D12 — references, never content), so the
        rebuilt line carries the fenced digest only — no decoded slots,
        no conformance report, no measured answer bytes: never a guess,
        ...
        charge = softconfig.budget_of(self.soft, self.mode, desk)
        cost_record = {"mode": self.mode, "charge": charge,
                       "measured": None,  # the answer's bytes are gone —
                                          # never a guessed measurement
                       "spend_before": spend_after - charge,
                       "spend_after": spend_after,
                       "ceiling": self.ceiling,
                       "answer_digest": record["payload_ref"]}
        return {
            "trail_version": TRAIL_VERSION,
            "scope": self.scope,
            "seq": self.trail.count,
            ...
            "source": "machine",
            "event": "turn",
            "signal": ("turn of %s at cycle %d — attended in live mode; "
                       "the ledger record landed while its trail line "
                       "did not (kill -9) — rebuilt from the fenced "
                       "digest alone, never a guessed decode (D12)"
                       % (desk, cycle)),
            "content": {
                "decoded": {},
...
class _NeutralLens:
    ...
    caller's deterministic stand-ins, never a live Pi.  The conductor's
    live mode speaks through the attested instrument path (prompt_desk),
    which carries its own centre guard and label assertions; this lens
    is not invoked there either (the B2 boot trust walk is not part of
    prompt_desk)."""
...
def seed_ref(source_ref, cell, cycle):
    ...
    the plant anchor's source ref — in live mode, the previous V turn's
    fenced payload_ref) to the seeding place — unique per (cell, cycle),
    so no two seeds share a payload_ref and the audit's chain stays
    exact."""
